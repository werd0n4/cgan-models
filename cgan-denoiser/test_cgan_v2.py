...
import artefacts_v2 as artefacts
import data_processing_v2 as data_processing
from config_v2 import ConfigCGAN as config
import cgan_v2 as model
...
def log_metric(value, name):
    with tf.compat.v2.summary.record_if(True):
        tf.compat.v2.summary.scalar(name=name, data=value, step=tf.compat.v1.train.get_or_create_global_step())
...
if __name__ == '__main__':
    ...
    summary_writer = tf.compat.v2.summary.create_file_writer(logdir=log_path, flush_millis=10000)
    ...
    generator_optimizer = tf.compat.v1.train.AdamOptimizer(config.learning_rate)
    discriminator_optimizer = tf.compat.v1.train.AdamOptimizer(config.learning_rate)
